...
class ClaimListCreateView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        # If the client already previewed the AI assessment on Step 4,
        # the result was submitted with the claim payload and is already
        # saved on the claim fields. Create the ClaimAssessment record
        # directly from those fields — no second Gemini call needed.
        if claim.ai_damage_summary:
            try:
                from .models import ClaimAssessment as CA
                CA.objects.filter(
                    claim=claim,
                    assessment_type=CA.AssessmentType.AI_INITIAL
                ).delete()
                CA.objects.create(
                    claim=claim,
                    assessor=None,
                    assessment_type=CA.AssessmentType.AI_INITIAL,
                    damage_description=claim.ai_damage_summary,
                    damage_severity='MINOR',
                    damaged_parts=[],
                    parts_cost=float(claim.ai_estimated_repair or 0),
                    labour_cost=0,
                    paint_cost=0,
                    other_costs=0,
                    total_estimate=float(claim.ai_estimated_repair or 0),
                    notes='Auto-generated by Gemini AI on submission.',
                    is_final=False,
                )
            except Exception:
                pass
        else:
            # No preview result — call Gemini fresh after documents are uploaded
            try:
...
class ClaimDocumentUploadView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        # Do NOT re-run AI assessment here — the result from the client's
        # Step 4 preview is already saved on the claim. Re-running would
        # overwrite it with a different answer.
